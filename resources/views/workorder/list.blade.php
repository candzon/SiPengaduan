...
@section('content')
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">List Pengaduan</h1>
        </div>
                    ...
                    @if (auth()->user()->role === 'admin')
                        <div>
                            ...
                            <select id="assigned-filter" name="user_id" onchange="handleStatusChange()"
                                class="shadow border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                <option value="All">All Pelapor</option>
            ...
            <table id="workOrderTable" class="min-w-full table-auto display">
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    ...
                    @foreach ($workOrders as $workorder)
                        @php
                            $pelapor = DB::table('users')->where('id', $workorder->user_id)->value('name');
                        @endphp
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="py-3 px-6 text-left whitespace-nowrap">
                                <span class="font-medium">{{ $rowNumber++ }}</span>
                            </td>
                            <td class="py-3 px-6 text-left">{{ $pelapor }}</td>
                            <td class="py-3 px-6 text-left">{{ $workorder->judul }}</td>
